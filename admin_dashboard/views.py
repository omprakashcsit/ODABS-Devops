...
def admin_patients(request):
    ...
    # Decrypt patient information only for display
    ...
    if search:
        ...
        patients = [
            ...
            for patient in patients
            if (
                search in (patient.name or "").lower()
                or search in (patient.email or "").lower()
                or search in (patient.phone or "").lower()
                or (
                    patient.user
                    and search in patient.user.username.lower()
                )
            )
        ]
...
def edit_patient(request, patient_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Keep linked User email synchronized
            if patient.user:
