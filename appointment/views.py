...
def generate_slots(doctor, selected_date, exclude_appointment=None):
    ...
    # Using friday's schedule for saturday. only for demo for defense
    if day == 5:      # Saturday
        day = 4       # Friday
...
def confirm_appointment(request):
    ...
    if request.method == "POST":
        ...
        # Get patient's previous appointment history

        previous_missed = Appointment.objects.filter(
            patient=request.user,
            status="missed"
        ).count()
        ...
        no_show_probability, no_show_prediction = predict_no_show(
            age=age,
            date_diff=date_diff,
       ...
       # Create appointment
        ...
        if approval_status == "approved":
            ...
            # High-risk notification + email
            if no_show_prediction:

                high_risk_message = (
                    "Your appointment has been identified as having "
                    "a higher risk of no-show. Please confirm your "
                    "attendance before your appointment."
                )
                ...
                if patient and patient.email:
                    ...
                    send_mail(
                        subject="Attendance Confirmation Required",
                        message=(
                            ...
                            f"Please log in to your account and confirm "
                            f"your attendance.\n\n"
                            f"Thank you,\n"
                            f"ODAS"
                        ),
...
def view_report(request, appointment_id):
    ...
    # Only the patient or assigned doctor can view the report
    if (
        request.user != appointment.patient
        and (
            not hasattr(request.user, "doctor")
            or appointment.doctor != request.user.doctor
        )
    ):
        messages.error(
            request,
...
def edit_report(request, appointment_id):
    ...
    # Decrypt existing report data so it can be displayed
    # in the edit form.
    if request.method == "GET":
        ...
        if form.is_valid():
            ...
            # Your MedicalReport model's save() method
            # should encrypt the sensitive fields.
            form.save()
...
def confirm_attendance(request, appointment_id):
    ...
    if not appointment.no_show_prediction:
        messages.warning(
            request,
            "Attendance confirmation is only required for high-risk appointments."
        )
        return redirect("patient_dashboard")
    ...
    if request.method == "POST":
        ...
        appointment.save(update_fields=["attendance_confirmed"])
...
@login_required
def view_report_file(request, appointment_id):
    ...
    # Only the patient or assigned doctor can access the file
    is_patient = request.user == appointment.patient

    is_doctor = (
        hasattr(request.user, "doctor")
        and appointment.doctor == request.user.doctor
    )
    ...
    # The plaintext file should never exist in media/
    if not report.encrypted_report_file:
        return HttpResponse(
            "No encrypted report file found.",
            status=404
        )
    ...
    try:
        ...
        print("FILE DECRYPTION ERROR:")
        print(type(e).__name__)
        print(str(e))

        return HttpResponse(
            "Unable to decrypt the medical report.",
            status=500
        )

    filename = (
        report.report_original_name
        or "medical_report"
    )
